...
const getLeftoverSections = async(
  ...
  let leftoverReadme = unified().use(remarkStringify).stringify(mdast)

  const tagline = await getTagline(leftoverReadme)

  leftoverReadme = leftoverReadme.replace(tagline, '')

  if (removeFirstImage) {
    const markdownAST = unified().use(remarkParse, { gfm: true }).parse(leftoverReadme)

    const imageIndex = markdownAST.children.findIndex((node) => {
      return (
        isParagraph(node) &&
        node.children.some((childNode) => {
          return isImage(childNode)
    ...
    if (imageIndex !== -1) {
      markdownAST.children.splice(imageIndex, 1)
      return unified().use(remarkStringify).stringify(markdownAST)
    }
  }

  return leftoverReadme
}
